...
export default {
        ...
        <main v-else class="page-roulette">
            <div class="sidebar">
                <p class="type-label-md" style="color: #aaa">
                    Cópia da Extreme Demon Roulette por <a href="https://matcool.github.io/extreme-demon-roulette/" target="_blank">matcool</a>.
                </p>
                <form class="options">
                    <div class="check">
                        <input type="checkbox" id="main" value="Main List" v-model="useMainList">
                        <label for="main">Lista principal</label>
                    </div>
                    <div class="check">
                        <input type="checkbox" id="extended" value="Extended List" v-model="useExtendedList">
                        <label for="extended">Lista extendida</label>
                    </div>
                    <Btn @click.native.prevent="onStart">{{ levels.length === 0 ? 'Start' : 'Restart'}}</Btn>
                </form>
                <p class="type-label-md" style="color: #aaa">
                    A roleta russa salva automáticamente.
                </p>
                <form class="save">
                    <p>Salvar/importar manualmente</p>
                    <div class="btns">
                        <Btn @click.native.prevent="onImport">Importar</Btn>
                        <Btn :disabled="!isActive" @click.native.prevent="onExport">Salvar</Btn>
                    </div>
                </form>
            </div>
    ...
    computed: {
        ...
        placeholder() {
            return `Porcentagem mínima ${this.currentPercentage + 1}%`;
        },
        hasCompleted() {
            return (
    ...
    methods: {
        ...
        async onStart() {
            if (this.isActive) {
                this.showToast('Desista antes de começar uma nova roleta.');
                return;
            }
            ...
            if (fullList.filter(([_, err]) => err).length > 0) {
                this.loading = false;
                this.showToast(
                    'A lista está em manutenção. Espere a manutenção terminar para começar uma nova roleta.',
                );
                return;
            }
        ...
        onDone() {
            ...
            if (
                ...
                this.showToast('Porcentagem inválida.');
                return;
            }
        ...
        onImport() {
            if (
                this.isActive &&
                !window.confirm('Isso irá apagar a roleta antiga, continuar?')
            ) {
                return;
            }
        ...
        async onImportUpload() {
            ...
            if (file.type !== 'application/json') {
                this.showToast('Arquivo inválido.');
                return;
            }
